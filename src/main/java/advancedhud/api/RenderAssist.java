...
//    }
    ...
    private static void drawSpriteUV(int x, int y, int width, int height, float u1, float v1, float u2, float v2)
    {
        ...
        VertexBuffer vertexBuffer = tessellator.getBuffer();
        vertexBuffer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
        vertexBuffer.pos(x, y + height, zLevel).tex(u1, v2).endVertex();
        vertexBuffer.pos(x + width, y + height, zLevel).tex(u2, v2).endVertex();
        vertexBuffer.pos(x + width, y, zLevel).tex(u2, v1).endVertex();
        vertexBuffer.pos(x, y, zLevel).tex(u1, v1).endVertex();
        tessellator.draw();
    }
