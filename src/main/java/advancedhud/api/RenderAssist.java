...
//    }
    ...
    private static void drawSpriteUV(int x, int y, int width, int height, float u1, float v1, float u2, float v2) {
        ...
        WorldRenderer worldRenderer = tessellator.getWorldRenderer();
        worldRenderer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
        worldRenderer.pos(x, y + height, zLevel).tex(u1, v2).endVertex();
        worldRenderer.pos(x + width, y + height, zLevel).tex(u2, v2).endVertex();
        worldRenderer.pos(x + width, y, zLevel).tex(u2, v1).endVertex();
        worldRenderer.pos(x, y, zLevel).tex(u1, v1).endVertex();
        tessellator.draw();
    }
